...
[App Server Supervisor](AUTO_RESEARCH_SUPERVISOR_SCHEDULER_DESIGN.md)。历史实现只作为
本文件中的演进背景，不再保留代码、CLI、配置或测试兼容路径。
...
早期曾考虑由 Goal 启动后台实验，暂停 Goal，由 Watcher 监听完成事件，再恢复 Goal 并注入实验结果。当时因为会话绑定、幂等和恢复接口尚未验证，没有采用。v0.3 曾验证 App Server 的 Goal API 和当前工具环境中的 thread id，并以 one-shot Listener 形式采用这一方向；它现已被 managed daemon Supervisor 取代。
...
flowchart LR
    ...
    E -->|"脱离 Desktop host<br/>统一 Goal scheduler"| F["v0.5<br/>managed daemon + Supervisor"]
...
| F | Codex Goal | CLI durable submit + Supervisor-owned Worker | v0.5，当前默认 |

这些阶段只用于解释设计取舍，不构成当前实现契约。
