...
class GoalSpec:
    ...
    def from_dict(cls, data: dict[str, Any]) -> GoalSpec:
        ...
        if not isinstance(hard_requirements, list):
            raise TypeError("hard_requirements must be a list")
        protocol_requirements = data.get(
            "protocol_requirements", constraints.get("protocol_requirements", [])
        )
        if not isinstance(protocol_requirements, list) or not all(
            isinstance(item, str) for item in protocol_requirements
        ):
            raise ValueError("protocol_requirements must be a list of strings")
        for requirement in hard_requirements:
            if not isinstance(requirement, dict):
                raise TypeError("each hard requirement must be an object")
            if not isinstance(requirement.get("metric"), str):
                raise TypeError("hard requirement metric must be a string")
            ...
            if not isinstance(value, (int, float)) or isinstance(value, bool):
                raise TypeError("hard requirement value must be numeric")
            when = requirement.get("when", {})
            if not isinstance(when, dict):
                raise TypeError("hard requirement when must be an object")
        return cls(
            ...
            editable_paths=search_space.get(
